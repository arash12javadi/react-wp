...
export default function PagesList({ onCreate, onEdit, role }: { onCreate: (isPost: boolean) => void; onEdit: (page: Page) => void; role: UserRole }) {
  ...
  return <section className={styles.container} aria-labelledby="pages-heading"><div className={styles.pageIntro}><div><h2 id="pages-heading">Pages &amp; Posts</h2><p>Manage static pages and blog posts from one content model.</p></div><div><button className={styles.primaryButton} onClick={() => onCreate(false)}>＋ New page</button> <button className={styles.primaryButton} onClick={() => onCreate(true)}>＋ New post</button></div></div>{error && <div className={styles.error} role="alert">{error}</div>}<div className={styles.toolbar}><select value={type} onChange={(e) => setType(e.target.value)}><option value="all">All content</option><option value="pages">Static pages</option><option value="posts">Posts</option><option value="templates">Templates (Page Builder)</option></select><select value={status} onChange={(e) => setStatus(e.target.value)}><option value="all">All statuses</option><option value="draft">Drafts</option><option value="published">Published</option></select></div><div className={styles.tableCard}>{loading ? <div className={styles.emptyState}>Loading content…</div> : <div className={styles.tableScroller}><table><thead><tr><th>Title</th><th>Type</th><th>Status</th><th>Updated</th><th>Actions</th></tr></thead><tbody>{pages.map((page) => <tr key={page.id}><td><strong>{page.title}</strong><span className={styles.slug}>/{page.slug}</span></td><td>{page.is_site_template ? 'Template' : page.is_post ? 'Post' : 'Page'}</td><td><button className={`${styles.status} ${page.status === 'published' ? styles.published : styles.draft}`} onClick={() => void toggle(page)}>{page.status}</button></td><td className={styles.date}>{new Date(page.updated_at).toLocaleDateString()}</td><td><button className={styles.editButton} onClick={() => onEdit(page)}>Edit</button>{rwp.getContentActions(page).map((action) => <a key={action.id} className={styles.editButton} href={action.href(page)}>{action.label}</a>)}<button className={styles.deleteButton} onClick={() => void remove(page)}>Delete</button></td></tr>)}</tbody></table></div>}</div></section>;
}
