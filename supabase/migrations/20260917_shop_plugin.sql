-- rwp-shop: a WooCommerce-style shop plugin.
--
-- Adds the Shop Manager role and the manage_shop capability, the shop tables, row level
-- security, and the functions that compute cart totals and place orders.
--
-- Prices, tax, shipping, coupons and stock are computed here, never in the browser: a
-- total sent from the client is attacker-controlled. The browser sends product ids and
-- quantities; shop_place_order() decides what they cost.
--
-- Safe to re-run.
...
-- Roles ------------------------------------------------------------------------
-- src/lib/roles.ts mirrors this. Change both or neither.
...
-- Fix: when the default_user_role option had never been saved, "null not in (...)" was
-- null rather than true, so the fallback never ran and every sign-up failed on a null role.
