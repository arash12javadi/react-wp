-- Chat system (plugins/rwp-chat). Safe to re-run.
--
-- This file is a copy of plugins/rwp-chat/schema.sql, which the Plugins screen runs on its own
-- when the chatbot is activated. It is here for sites that prefer to run migrations by hand in
-- the Supabase SQL Editor, and for sites that installed the plugin before it shipped a schema.
--
-- Change one and the other must get the same text: everything below this line is that copy.
